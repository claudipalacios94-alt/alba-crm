// ══════════════════════════════════════════════════════════════
// ALBA CRM — INDICADOR DE CONEXIÓN SUPABASE
// ══════════════════════════════════════════════════════════════
...
export default function SupabaseStatus({ loading, error, lastSync, onReload }) {
  ...
  if (error) return (
    <div style={{ display:"flex", alignItems:"center", gap:5, padding:"5px 8px",
      ...
      <button onClick={onReload} style={{ background:"transparent", border:"none",
        color:B.hot, cursor:"pointer", fontSize:11, padding:0, flexShrink:0 }}>↻</button>
    </div>
  );
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:5, padding:"5px 8px",
      ...
      <button onClick={onReload} style={{ background:"transparent", border:"none",
        color:"#2E9E6A", cursor:"pointer", fontSize:11, padding:0, flexShrink:0 }}>↻</button>
    </div>
  );
}
